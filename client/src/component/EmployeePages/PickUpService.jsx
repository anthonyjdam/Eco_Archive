...
export default function PickUpService() {
  ...
  function handleCompleteOrder(e) {
    axios
      .get(
        `http://localhost:5000/api/recyclable/${state.customer.RecyclableName}`
      )
      .then((response) => {
        const completeStatus = {
          ...
          recyclablename: state.customer.RecyclableName,
          amountofmaterialsgiven: state.customer.AmountOfMaterialsGiven,
          amountearned:
            state.customer.AmountOfMaterialsGiven *
            response.data[0].MaterialRate,
        ...
        axios.post('http://localhost:5000/api/updateInventory', {
            materialtype: response.data[0].MaterialType,
            materialcount: state.customer.AmountOfMaterialsGiven,
            branchname: state.customer.BranchName,
        }).catch(() => {
            console.log("error");
        });
  ...
  return (
    <main className="flex flex-col gap-3 relative m-1 px-3 lg:pl-1 min-h-screen bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-sky-50 via-indigo-100 to-emerald-50 ">
      ...
      <div>
        <h1 className="sm:text-4xl lg:text-5xl px-2 font-semibold text-4xl text-center .text-gray-700 py-10">
          PickUp <span className="font-extrabold">Service</span>
        </h1>
      </div>
      ...
      <div className="flex flex-col gap-2 relative m-1 lg:min-w-screen">
        <div className="px-7 pt-3 pb-4 rounded-sm border-8 border-gray-500 flex-1 text-center">
              ...
              <div class="flex justify-items-start mb-1">
                ...
                <div class="w-2/4">
                  <span
                    ...
                    disabled
                  >
                    {new Date(state.customer.DateTime).toDateString()}{" "}
                  </span>
                </div>
              </div>
              ...
              <div class="flex justify-items-start mb-1">
                ...
                <div class="w-2/4">
                  <input
                    ...
                    id="inline-emp-id"
                    type="number"
                    placeholder={transactionTotal === 0? "####" : transactionTotal}
                    disabled
                  >
          ...
          <div className="text align-middle flex flex-col justify-items-center px-16 lg:px-72">
            <label
              for="workstation"
              class="font-semibold text-sky-800 text-center"
            >
              Submission Summary
            </label>
          </div>
          ...
          <div className="text align-middle flex flex-col justify-items-center font-semibold font-mono py-4 px-12 lg:px-80 ">
            ...
            <span
              type="number"
              name="Glass"
              className="border-2 bg-white rounded-lg text-center font-normal"
            >
              {state.customer.RecyclableName}{" "}
            </span>
            ...
            <span
              type="number"
              name="Tin"
              className="rounded-lg text-center border-2 bg-white"
            >
              {" "}
              {state.customer.AmountOfMaterialsGiven}{" "}
            </span>
            ...
            <button
              className="border border-sky-900 mt-7 bg-orange-300 rounded-sm"
              onClick={(e) => {
